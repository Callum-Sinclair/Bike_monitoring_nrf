...
#define OPCODE_LENGTH 1                                                    /**< Length of opcode inside USR Measurement packet. */
#define HANDLE_LENGTH 2                                                    /**< Length of handle inside USR Measurement packet. */
#define MAX_CSCM_LEN  (BLE_L2CAP_MTU_DEF - OPCODE_LENGTH - HANDLE_LENGTH)  /**< Maximum size of a transmitted USR Measurement. */
 ...
 */
static void on_connect(ble_usr_t * p_usr, ble_evt_t * p_ble_evt)
{
    p_usr->conn_handle = p_ble_evt->evt.gap_evt.conn_handle;
}
 ...
 * @param[in]   p_ble_evt   Event received from the BLE stack.
 */
static void on_disconnect(ble_usr_t * p_usr, ble_evt_t * p_ble_evt)
{
    UNUSED_PARAMETER(p_ble_evt);
    p_usr->conn_handle = BLE_CONN_HANDLE_INVALID;
}
 ...
 */
static void on_meas_cccd_write(ble_usr_t * p_usr, ble_gatts_evt_write_t * p_evt_write)
{
    if (p_evt_write->len == 2)
    {
        // CCCD written, update notification state
        if (p_usr->evt_handler != NULL)
        {
            ble_usr_evt_t evt;
 ...
 * @param[in]   p_ble_evt   Event received from the BLE stack.
 */
static void on_write(ble_usr_t * p_usr, ble_evt_t * p_ble_evt)
{
    ble_gatts_evt_write_t * p_evt_write = &p_ble_evt->evt.gatts_evt.params.write;

    if (p_evt_write->handle == p_usr->meas_handles.cccd_handle)
    {
        on_meas_cccd_write(p_usr, p_evt_write);
    }
}
...
{
    ble_sc_ctrlpt_on_ble_evt(&(p_usr->ctrl_pt), p_ble_evt);

    switch (p_ble_evt->header.evt_id)
    {
        case BLE_GAP_EVT_CONNECTED:
            on_connect(p_usr, p_ble_evt);
            break;

        case BLE_GAP_EVT_DISCONNECTED:
            on_disconnect(p_usr, p_ble_evt);
            break;

        case BLE_GATTS_EVT_WRITE:
            on_write(p_usr, p_ble_evt);
            break;
 ...
 */
static uint8_t csc_measurement_encode(ble_usr_t      * p_usr,
                                      ble_usr_meas_t * p_usr_measurement,
                                      uint8_t        * p_encoded_buffer)
{
    ...
    if (p_usr->feature & BLE_CSCS_FEATURE_WHEEL_REV_BIT)
    {
        if (p_usr_measurement->is_wheel_rev_data_present)
        {
    ...
    if (p_usr->feature & BLE_CSCS_FEATURE_CRANK_REV_BIT)
    {
        if (p_usr_measurement->is_crank_rev_data_present)
        {
            flags |= CSC_MEAS_FLAG_MASK_CRANK_REV_DATA_PRESENT;
            len += uint16_encode(p_usr_measurement->cumulative_crank_revs, &p_encoded_buffer[len]);
            len += uint16_encode(p_usr_measurement->last_crank_event_time, &p_encoded_buffer[len]);
        }
    }
