...
/** @file
 *
 * @defgroup ble_sdk_srv_usr USR Service
 * @{
 * @brief USR Service module.
 *
 * @details FALSE This module implements the Cycling Speed and Cadence Service. If enabled, notification
...
/**@brief USR Service event type. */
typedef enum
{
    BLE_USR_EVT_NOTIFICATION_ENABLED,                                  /**< Cycling Speed and Cadence value notification enabled event. */
    BLE_USR_EVT_NOTIFICATION_DISABLED                                  /**< Cycling Speed and Cadence value notification disabled event. */
} ble_usr_evt_type_t;

/**@brief Cycling Speed and Cadence Service event. */
typedef struct
{
    ble_usr_evt_type_t evt_type;                                       /**< Type of event. */
} ble_usr_evt_t;

// Forward declaration of the ble_usr_t type. 
typedef struct ble_usr_s ble_usr_t;

/**@brief USR Service event handler type. */
typedef void (*ble_usr_evt_handler_t) (ble_usr_t * p_cscs, ble_usr_evt_t * p_evt);

/**@brief USR Service init structure. This contains all options and data
*         needed for initialization of the service. */
typedef struct
{
    ble_usr_evt_handler_t        evt_handler;                           /**< Event handler to be called for handling events in the Cycling Speed and Cadence Service. */
    ble_srv_cccd_security_mode_t usr_meas_attr_md;                      /**< Initial security level for cycling speed and cadence measurement attribute */
    ble_srv_cccd_security_mode_t usr_ctrlpt_attr_md;                    /**< Initial security level for cycling speed and cadence control point attribute */
    ble_srv_security_mode_t      sur_feature_attr_md;                   /**< Initial security level for feature attribute */
...
/**@brief USR Service structure. This contains various status information for
 *        the service. */
struct ble_usr_s
{
    ble_usr_evt_handler_t        evt_handler;                           /**< Event handler to be called for handling events in the Cycling Speed and Cadence Service. */
...
/**@brief USR Service measurement structure. This contains a Cycling Speed and
 *        Cadence Service measurement. */
typedef struct ble_usr_meas_s
{
    uint16_t    distance;                                               /**< Distance to vehicle detected by USR. */
} ble_usr_meas_t;
 ...
 */
uint32_t ble_cscs_init(ble_usr_t * p_usr, const ble_usr_init_t * p_usr_init);
 ...
 */
void ble_cscs_on_ble_evt(ble_usr_t * p_usr, ble_evt_t * p_ble_evt);
 ...
 */
uint32_t ble_cscs_measurement_send(ble_usr_t * p_usr, ble_usr_meas_t * p_measurement);
