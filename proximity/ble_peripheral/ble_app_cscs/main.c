...
#define BAT_PIN                         7 // batery voltage measuring analgoue pin
...
static ble_usr_service_t                     m_xxs;
static ble_uuid_t m_adv_uuids[] = {{BLE_UUID_USR_SERVICE, BLE_UUID_TYPE_BLE}}; /**< Universally unique service identifiers. */
...
static void battery_level_update(void)
{
    ...
    battery_level = (uint8_t)battery_measure(BAT_PIN);
...
static void csc_meas_timeout_handler(void * p_context)
{
    ...
    uint16_t usr_measurement = usr_measure();

    err_code = ble_usr_measurement_send(&m_usr, &usr_measurement);
    if ((err_code != NRF_SUCCESS) &&
        (err_code != NRF_ERROR_INVALID_STATE) &&
        (err_code != BLE_ERROR_NO_TX_PACKETS) &&
        (err_code != BLE_ERROR_GATTS_SYS_ATTR_MISSING)
...
{
    uint32_t              err_code;
    ble_cscs_init_t        usr_init;
    ble_bas_init_t        bas_init;
    ble_dis_init_t        dis_init;

    // Initialize Cycling Speed and Cadence Service.
    memset(&usr_init, 0, sizeof(usr_init));

    usr_init.evt_handler = NULL;
    usr_init.feature     = BLE_CSCS_FEATURE_WHEEL_REV_BIT | BLE_CSCS_FEATURE_CRANK_REV_BIT |
    ...
    BLE_GAP_CONN_SEC_MODE_SET_OPEN(&usr_init.csc_meas_attr_md.cccd_write_perm);   // for the measurement characteristic, only the CCCD write permission can be set by the application, others are mandated by service specification
    BLE_GAP_CONN_SEC_MODE_SET_OPEN(&usr_init.csc_feature_attr_md.read_perm);      // for the feature characteristic, only the read permission can be set by the application, others are mandated by service specification
    BLE_GAP_CONN_SEC_MODE_SET_OPEN(&usr_init.csc_ctrlpt_attr_md.write_perm);      // for the SC control point characteristic, only the write permission and CCCD write can be set by the application, others are mandated by service specification
    BLE_GAP_CONN_SEC_MODE_SET_OPEN(&usr_init.csc_ctrlpt_attr_md.cccd_write_perm); // for the SC control point characteristic, only the write permission and CCCD write can be set by the application, others are mandated by service specification
    
    usr_init.ctrplt_supported_functions    = BLE_SRV_SC_CTRLPT_CUM_VAL_OP_SUPPORTED
                                              |BLE_SRV_SC_CTRLPT_SENSOR_LOCATIONS_OP_SUPPORTED
                                              |BLE_SRV_SC_CTRLPT_START_CALIB_OP_SUPPORTED;
    usr_init.ctrlpt_evt_handler            = sc_ctrlpt_event_handler;
    usr_init.list_supported_locations      = supported_locations;
    usr_init.size_list_supported_locations = sizeof(supported_locations) / sizeof(ble_sensor_location_t);            
